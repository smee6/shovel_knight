...
HRESULT mHellFire::init(int bulletMax, float range)
{
	_bulletMax = bulletMax;
	_range = range;
...
void mHellFire::release()
{
}

void mHellFire::update()
{
	move();
}

void mHellFire::fire(float x, float y, float angle)
{
	//만약에 벡터의 사이즈가 총알 최대치보다 커지려고 하면 
	if (_bulletMax < _vBullet.size()) return;
...
void mHellFire::removeMissile(int arrNum)
{
	_vBullet[arrNum].bulletImage->release();
	_vBullet.erase(_vBullet.begin() + arrNum);
}

void mHellFire::render()
{
	for (_viBullet = _vBullet.begin(); _viBullet != _vBullet.end(); ++_viBullet)
	{
		_viBullet->bulletImage->frameRender(getMemDC(),
